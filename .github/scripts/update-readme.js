...
newReadme += sortedDifficultyKeys.map(difficulty => `| ${difficulty} | ${difficulties[difficulty].length} |`).join('\n');
newReadme += '\n\n';
newReadme += `### Total Solved: ${directories.length}\n\n`;
newReadme += '## Problems\n\n';

newReadme += sortedDifficultyKeys.map(difficulty => {
  ...
  const tableHeader = `| 번호 | 제목 |\n| --- | --- |`;
  const tableRows = problems
    .map(p => `| ${p.problemNumber || '-'} | [${p.title}](${p.folderLink}) |`)
    .join('\n');
...
console.log('루트 README.md가 업데이트되었습니다.');
